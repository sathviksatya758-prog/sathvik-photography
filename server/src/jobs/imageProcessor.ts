...
// The actual image-processing work, factored out of the BullMQ worker so it
// can run either in the dedicated worker process (Redis present) or inline in
// the API process (no Redis). See lib/queue.ts::enqueueImageJob.
...
export async function processImageJob(data: ImageJobData, attempt = 1): Promise<void> {
  const { photoId, storageKey } = data;
  logger.info({ photoId, attempt }, 'image job started');
  ...
  await prisma.$transaction(async tx => {
    ...
    const writeData = photoAiWriteData(ai);
    await tx.photoAi.upsert({
      where: { photoId },
      create: { photoId, ...writeData },
      update: { ...writeData, generatedAt: new Date() }
    });
...
async function generateSuggestions(photoId: string, ai: AiMeta): Promise<void> {
  ...
  const duplicates = await prisma.$queryRawUnsafe<{ photo_id: string; score: number }[]>(
    `SELECT * FROM find_duplicate_photos($1::uuid, $2::real)`,
    photoId,
